function [hImage, hText, hXText] = heatmap(mat, xlab, ylab, varargin)
...
%   displayed in each squre, a format string, in which case the matrix
...
muVal = squeeze(nanmean(mat,1));
if length(size(mat))==2, muVal = mat; end
ceilVal = max(abs([max(muVal(:)),min(muVal(:))]));
...
if p.decal,
    p.MinColorValue = -ceilVal;
    p.MaxColorValue = ceilVal;
end

if p.oriVer,
    for i1 = 1:size(mat_orig,3),
        mat(:,i1) = circ_mean_ori(mat_orig(:,:,i1),[],1);
    end
end
...
% if length(size(p.alpha))~=1,
%    
%    
%    [k,xn,yn] = size(mat_orig);
%    matAlpha = zeros(k,xn,yn);
%    for m = 1:k,
%        kmat = mat_orig(m,:,:);
%        kmat_filt = nan(1,xn,yn);
%        kmat_filt(find(p.alpha)) = kmat(find(p.alpha));
%        matAlpha(m,:,:) = kmat_filt;
%    end
%    
%    muAlphaVal = squeeze(mean(matAlpha,1));
%    
%    %contourf(muAlphaVal,1)
%    
%    hold on;
%    p = plotHeatmap(p, muAlphaVal); % New properties hImage and cdata added
% end
...
hold on;
if p.saveMode
    xx = [];for xi = 1:length(xlab),xx = [xx,str2num(xlab{xi})]; end
    yy = [];for yi = 1:length(ylab),yy = [yy,str2num(ylab{yi})]; end
    [x,y] = meshgrid(xx,yy);
    x = x(:);
    y = y(:);
    x(p.alpha(:)==1)=[];
    ...
    col = [1,1,1]*0;
    p2 = plot(x,y,'o','Color','none','MarkerSize',1,'MarkerFaceColor',col);
end
set(p.hImage,'AlphaData',p.alpha*0.7+0.3)
if mean(p.alpha(:))<1, contour((p.alpha),1,'Color',[0,0,0]); end
hold off;
%% %% %% %% %% %% %% %% %%
end
...
p = inputParser;
p.addParamValue('Colormap','jet'); %#ok<*NVREPL>
...
p.addParamValue('alpha', 1, @(x)ismatrix(x) || isvector(x));
p.addParamValue('decal', 0, @(x)isnumeric(x));
p.addParamValue('oriVer', 0, @(x)isnumeric(x));
p.addParamValue('saveMode', 0, @(x)isnumeric(x));
p.parse(varargin{:});
